...
| `GET /api/models` | Enumerate upscale models with their runtime (`backend`: `torch` \| `ncnn`), the dropdown header they sit under (`group`), and for Vulkan models the GPU VRAM tiers (`tile_presets`, whose `tile` the client writes into the existing `tile_size` setting) |
