...
| `POST /api/pdf/cut-file` | The SVG cut file for a sheet with cutter registration marks: every grid cell's rounded trim box plus the marks, in the cutter's own orientation (`pdf_layout.cutter_frame`), for import into Silhouette Studio. Same `PdfLayoutIn` body as the preview but pure geometry — reads no gallery, needs no generated images; 400 when no cutter is selected |
