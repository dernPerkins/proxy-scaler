...
export const PATCH_NOTES: PatchNotesEntry[] = [
  {
    ...
    date: "September 2026",
    notes: [
      "Bleed now fans out of the rounded corners — every bleed pixel takes the colour of the nearest point on the card's edge, so corners spread radially instead of filling a square block, and the four-card junctions on a sheet look like a professional print.",
      "Gold-bordered cards (Pro Tour Collector Set and the World Championship decks) print cleanly: the white arc on every corner, the white bars stretched out of the corners into the bleed, and the white streak across the top and bottom bleed are gone.",
      "Every edge defect Scryfall's renders carry — the phantom dark row on some white cards, a phantom white column on a Secret Lair print, the gold template's rim — is handled by one rule instead of one fix each, so odd edges no longer need a colour signature the code has to recognise.",
      "The card's visible edge sits exactly on the cut guide again at every bleed size; the guide's inner edge and the trim line land on the same pixel.",
      "Custom images that already include bleed (MakePlayingCards downloads, for example): tick \"This image already includes bleed\" in the new Customs sidebar and enter the amount (3.175 mm by default). Printing uses the file's own bleed up to the project's setting and only generates bleed beyond it, so the card is no longer shrunk with its bleed baked in.",
      "Back images gain the same bleed amount next to their existing checkbox, so a pre-bled back prints at exactly card size whatever bleed the project uses. The back-page preview now honours that setting as well.",
      "The Customs tab has a settings sidebar for the selected image — name, bleed declaration, and Remove — matching the Backs tab. Changing an image's bleed re-uploads it and discards its old upscales, so generate again afterwards.",
      "Custom images that include bleed need a 0.3.3 server; the app says so rather than printing a wrong card against an older one.",
    ],
  },
  {
    version: "0.3.2",
